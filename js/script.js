...
function generatePOCard(po) {
  ...
  card.innerHTML = `
    <h3>Here are the details of the provided ERP number :</h3>
    <div class="po-bold-value">${po.documentNumber}</div>
    <div class="sep-line"></div>
    <div class="bot-po-footer-text">
      Would you like to Create ASN of the provided PO ?
    </div>
...
async function fetchPODetails(docNum) {
  try {
    ...
    const po = (json.resource || []).find(p => p.documentNumber === docNum);
    if (!po) {
      addBotBubble("Provided PO is invalid.");
      return;
    }
    ...
    const wrapper = document.createElement("div");
    wrapper.className = "bot-message-wrapper";
    wrapper.appendChild(generatePOCard(po));
    chatMessages.appendChild(wrapper);
    chatMessages.scrollTop = chatMessages.scrollHeight;
...
/*******************************
 ASN BACKEND
*******************************/
async function callASNBackend(text, poNumber) {
  const res = await fetch("http://127.0.0.1:8000/asn/direct", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ poNumber, userText: text })
  });

  const data = await res.json();
  addBotBubble(data.error ? data.error : "ASN payload generated successfully.");
  window.asnMode = false;
}
